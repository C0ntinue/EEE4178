...
rnn_types = ['RNN','LSTM','GRU']  # select from 'RNN', 'LSTM' and 'GRU'.
...
def plot_losses(train_losses, dropout, num_epochs, batch_size,rnn_type):
    ...
    ax.set(title=rnn_type+"\'s Loss over 100 data samples, using dropout =" + str(dropout) + ",epoch = " + str(
        num_epochs) \
                        + ","
                                                                                                      "batch_size = "
                                                                                                      "" + str(
    ...
    ax.legend()
    plt.tight_layout()
    fig.savefig('./Train_Results/loss.png', dpi=300)
    # fig.show()
...
# optimizer = torch.optim.Adam(test_model.parameters(), lr=learning_rate, weight_decay=learning_rate * 0.01,
#                              amsgrad=True)
