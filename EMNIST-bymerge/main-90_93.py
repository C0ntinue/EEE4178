...
class SpinalVGG(nn.Module):
    ...
    def __init__(self, num_classes=47, p=0):
        ...
        self.fc_spinal_layer1 = nn.Sequential(
            nn.Dropout(p), nn.Linear(Half_width, layer_width),
            nn.BatchNorm1d(layer_width), nn.ReLU(inplace=True), )
        self.fc_spinal_layer2 = nn.Sequential(
            nn.Dropout(p), nn.Linear(Half_width + layer_width, layer_width),
            nn.BatchNorm1d(layer_width), nn.ReLU(inplace=True), )
        ...
        self.fc_out = nn.Sequential(
            nn.Dropout(p), nn.Linear(layer_width * 4, num_classes), nn.LogSoftmax(dim=1))
        self.fc_label = nn.Sequential(
            nn.Dropout(p), nn.Linear(num_classes, 2), )
...
scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=1, mode='min', eps=1e-20)
